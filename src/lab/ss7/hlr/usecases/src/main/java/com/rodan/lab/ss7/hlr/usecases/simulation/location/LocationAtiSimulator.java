...
public class LocationAtiSimulator extends Ss7SimulatorTemplate implements SignalingModule, MapMobilityServiceListener {
    ...
    public void onAnyTimeInterrogationRequest(AtiRequest request) throws SystemException {
        try {
            ...
            notify("Received ATI request for MSISDN: " + msisdn + ", from gsmSCF: " + gsmScf, NotificationType.PROGRESS);

            var dialog = request.getDialog();
            var invokeId = request.getInvokeId();
            dialog.setUserObject(invokeId);
            var authorizedNodeGt = moduleOptions.getNodeConfig().getTargetNetwork().getGsmScfGt();
            if (authorizedNodeGt.equals(gsmScf)) {
                ...
                // TODO send MAP retutnError with ati-NotAllowed (49) code instead of ReturnErrorUnexpected
