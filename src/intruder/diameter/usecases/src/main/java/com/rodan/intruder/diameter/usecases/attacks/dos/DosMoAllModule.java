...
import com.rodan.intruder.diameter.usecases.model.dos.DosMoAllOptions;
import com.rodan.intruder.diameter.usecases.model.dos.DosMoAllResponse;
...
@Module(name = DiameterModuleConstants.DOS_MO_ALL_NAME, category = DiameterModuleConstants.DOS_CATEGORY_DISPLAY_NAME,
        displayName = DiameterModuleConstants.DOS_MO_ALL_DISPLAY_NAME, brief = DiameterModuleConstants.DOS_MO_ALL_BRIEF,
        description = DiameterModuleConstants.DOS_MO_ALL_DESCRIPTION, rank = DiameterModuleConstants.DOS_MO_ALL_RANK,
        mainPayload = Constants.IDR_PAYLOAD_NAME)
public class DosMoAllModule extends DiameterModuleTemplate implements SignalingModule, S6aListener {
    final static Logger logger = LogManager.getLogger(DosMoAllModule.class);

    @Builder
    public DosMoAllModule(DiameterGateway gateway, DiameterModuleOptions moduleOptions) {
    ...
    protected void generatePayload() {
        ...
        var options = (DosMoAllOptions) moduleOptions;
        var payload = IdrPayload.builder()
                .usage(IdrPayload.Usage.BAR_ODB)
    ...
    public void doInsertSubscriberDataAnswerEvent(InsertSubscriberDataAnswer answer) {
        ...
        moduleResponse = DosMoAllResponse.builder()
