...
import com.rodan.intruder.diameter.usecases.model.fraud.FraudAccessRestrictionResponse;
...
@Module(name = DiameterModuleConstants.FRAUD_ACCESS_RESTRICTION_NAME, category = DiameterModuleConstants.FRAUD_CATEGORY_DISPLAY_NAME,
        displayName = DiameterModuleConstants.FRAUD_ACCESS_RESTRICTION_DISPLAY_NAME, brief = DiameterModuleConstants.FRAUD_ACCESS_RESTRICTION_BRIEF,
        description = DiameterModuleConstants.FRAUD_ACCESS_RESTRICTION_DESCRIPTION, rank = DiameterModuleConstants.FRAUD_ACCESS_RESTRICTION_RANK,
        mainPayload = Constants.IDR_PAYLOAD_NAME)
public class FraudAccessRestrictionModule extends DiameterModuleTemplate implements SignalingModule, S6aListener {
    final static Logger logger = LogManager.getLogger(FraudAccessRestrictionModule.class);

    @Builder
    public FraudAccessRestrictionModule(DiameterGateway gateway, DiameterModuleOptions moduleOptions) {
    ...
    protected void generatePayload() {
        ...
        var options = (FraudAccessRestrictionOptions) moduleOptions;
        var payload = IdrPayload.builder()
                .usage(IdrPayload.Usage.UNBAR_ACCESS_RESTRICTION)
    ...
    public void doInsertSubscriberDataAnswerEvent(InsertSubscriberDataAnswer answer) {
        ...
        moduleResponse = FraudAccessRestrictionResponse.builder()
