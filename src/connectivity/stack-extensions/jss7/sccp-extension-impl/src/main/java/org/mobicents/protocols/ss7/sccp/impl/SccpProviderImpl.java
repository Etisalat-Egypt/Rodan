...
public class SccpProviderImpl implements SccpProvider, Serializable {
    ...
    protected FastList<SccpSstListener> sstListener = new FastList<SccpSstListener>();
    ...
    public void registerSccpSstListener(SccpSstListener listener) {
        synchronized (this) {
            if (this.sstListener.contains(listener))
                return;

            FastList<SccpSstListener> newListener = new FastList<SccpSstListener>();
            newListener.addAll(this.sstListener);
            newListener.add(listener);
            this.sstListener = newListener;
        }
    }

    public void deregisterSccpSstListener(SccpSstListener listener) {
        synchronized (this) {
            if (!this.sstListener.contains(listener))
                return;
    ...
    protected FastList<SccpSstListener> getAllSccpSstListeners() {
        return sstListener;
    }
