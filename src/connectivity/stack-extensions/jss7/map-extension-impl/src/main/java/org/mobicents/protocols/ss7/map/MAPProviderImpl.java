...
//    protected transient FastMap<String, String> congSources = new FastMap<String, String>();
    ...
    public MAPProviderImpl(String name, TCAPProvider tcapProvider) {
        ...
        // ########### TRX: PATCH START
        this.isLabMode = false;
        // ########### TRX: PATCH END
    }

    // ########### TRX: PATCH START
    public void setIsLabMode(boolean isLabMode) {
        this.isLabMode = isLabMode;
    }
    // ########### TRX: PATCH END
...
//    }
    ...
    private void processComponents(MAPDialogImpl mapDialogImpl, Component[] components) {
        // Now let us decode the Components
        // ########### TRX: PATCH START
        if (isLabMode && components.length > 1) {
            loger.debug("Rejecting TCAP dialog with multiple components.");
            doProcessRejectedComponent(mapDialogImpl, components[0]);
        }
         else {
            // ########### TRX: PATCH END
            loger.debug("Processing " + components + "components.");
             for (Component c : components) {
                 loger.debug("Processing component: " + c);
    ...
    // ########### TRX: PATCH START
    private void doProcessRejectedComponent(MAPDialogImpl mapDialogImpl, Component c) {
        // Getting the MAP Service that serves the MAP Dialog
        MAPServiceBaseImpl perfSer = (MAPServiceBaseImpl) mapDialogImpl.getService();
        try {
            ...
            switch (compType) {
                case Invoke:
                    type = ProblemType.Invoke;
                    break;
                case ReturnResultLast:
                case ReturnResult:
                    type = ProblemType.ReturnResult;
                    break;
                case ReturnError:
                    type = ProblemType.ReturnError;
                    break;
                default:
                    return;
            }
            ...
            loger.error("Error processing a Rejected Component: " + e.getMessage() + "\nComponent" + c, e);
        }
    }
    // ########### TRX: PATCH END
    ...
    private void doProcessComponent(MAPDialogImpl mapDialogImpl, Component c) {
        ...
        // Getting the MAP Service that serves the MAP Dialog
        MAPServiceBaseImpl perfSer = (MAPServiceBaseImpl) mapDialogImpl.getService();
        loger.debug("Handling component [" + c + "] using service: [" + perfSer + "]");
...
//                    .tail(); (n = n.getNext()) != end;) {
    ...
    protected TCBeginRequest encodeTCBegin(Dialog tcapDialog, ApplicationContextName acn, AddressString destReference,
        ...
        if ((destReference != null || origReference != null || mapExtensionContainer != null || eriStyle)
            ...
            userInformation.setOidValue(MAPDialogueAS.MAP_DialogueAS.getOID()); // TODO IMP TRX: check if this can overrides malformed ACN
