...
@Payload(name = Constants.SAI_PAYLOAD_NAME)
@ToString(callSuper = true)
public class SaiPayloadWrapper extends JSs7PayloadWrapper<MapMobilityService, MAPDialogMobility> {
    ...
    @Builder
    public SaiPayloadWrapper(String localGt, int localSsn, int remoteSsn, NodeConfig nodeConfig, SccpAdapter sccpAdapter, 
                             MapAdapter mapAdapter, MapDialogGenerator<MAPDialogMobility> dialogGenerator, String imsi, 
                             String msisdn, String targetHlrGt, String avNumber, String mapVersion) {
    ...
    public MAPDialogMobility generateCarrier() throws SystemException {
        ...
        var calledGtStr = StringUtils.isBlank(getTargetHlrGt()) ? getMsisdn() :
                getTargetHlrGt();
        var calledGt = paramFactory.createGlobalTitle(calledGtStr, TRANSLATION_TYPE, ISDN_TELEPHONY_INDICATOR,
    ...
    public void addToCarrier(MAPDialogMobility dialog) throws SystemException {
        try {
            ...
            // TODO IMP TRX: change immediate to true and test
            dialog.addSendAuthenticationInfoRequest(imsi, avNumber, false, false,
                    null, null, RequestingNodeType.vlr, null,
                    null, false);

        } catch (MAPException e) {
            logger.error("Failed to add SAI to dialog", e);
    ...
    protected MAPApplicationContext getApplicationContext() throws SystemException {
        ...
        return MAPApplicationContext.getInstance(MAPApplicationContextName.infoRetrievalContext, mapContextVersion);
    }
}
