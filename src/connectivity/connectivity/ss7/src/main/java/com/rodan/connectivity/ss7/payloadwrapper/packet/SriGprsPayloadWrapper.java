...
package com.rodan.connectivity.ss7.payloadwrapper.packet;
...
import com.rodan.connectivity.ss7.service.MapPdpService;
...
@Payload(name = Constants.SRI_GPRS_PAYLOAD_NAME)
@ToString(callSuper = true)
public class SriGprsPayloadWrapper extends JSs7PayloadWrapper<MapPdpService, MAPDialogPdpContextActivation> {
    ...
    @Getter(AccessLevel.PRIVATE) private String ggsnNumber;
    @Getter(AccessLevel.PRIVATE) private String mapVersion;

    @Builder
    public SriGprsPayloadWrapper(String localGt, int localSsn, int remoteSsn, NodeConfig nodeConfig, 
                                 SccpAdapter sccpAdapter, MapAdapter mapAdapter, 
                                 MapDialogGenerator<MAPDialogPdpContextActivation> dialogGenerator, String imsi, 
                                 String msisdn, String targetHlrGt, String ggsnNumber, String mapVersion) {
    ...
    @Override
    public MAPDialogPdpContextActivation generateCarrier() throws SystemException {
        validate();
    ...
    @Override
    public void addToCarrier(MAPDialogPdpContextActivation dialog) throws SystemException {
        try {
            validate();

            var mapParamFactory = getMapAdapter().getParamFactory();
            var imsi =mapParamFactory.createIMSI(getImsi());
            var ggsnGt = StringUtils.isBlank(getGgsnNumber()) ? getLocalGt() :
                    getGgsnNumber();
            var ggsnNumber = mapParamFactory.createISDNAddressString(AddressNature.international_number,
                    NumberingPlan.ISDN, ggsnGt);

            // ggsnAddress is the IP address of GGSN
            dialog.addSendRoutingInfoForGprsRequest(imsi,null, ggsnNumber, null);

        } catch (MAPException e) {
            logger.error("Failed to add SRI_GPRS to dialog", e);
    ...
    protected MAPApplicationContext getApplicationContext() throws SystemException {
        Integer[] supportedVersions = {3, 4};
        var mapContextVersion = getAcVersion(Integer.parseInt(getMapVersion()), supportedVersions);
        return MAPApplicationContext.getInstance(MAPApplicationContextName.gprsLocationInfoRetrievalContext, mapContextVersion);
    }
}
