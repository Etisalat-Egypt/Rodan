...
@Payload(name = Constants.ATI_PAYLOAD_NAME)
@ToString(callSuper = true)
public class AtiPayloadWrapper extends JSs7PayloadWrapper<MapMobilityService, MAPDialogMobility> {
    ...
    @Getter(AccessLevel.PRIVATE) private String gsmSCFGt; // GSM Service Control Function (CAMEL)
    ...
    @Builder
    public AtiPayloadWrapper(String localGt, int localSsn, int remoteSsn, NodeConfig nodeConfig, SccpAdapter sccpAdapter, 
                             MapAdapter mapAdapter, MapDialogGenerator<MAPDialogMobility> dialogGenerator, String msisdn, 
                             String imsi, String targetHlrGt, String gsmSCFGt, String abuseOpcodeTag, String malformedAcn, 
                             String mapVersion) {
    ...
    public MAPDialogMobility generateCarrier() throws SystemException {
        ...
        var callingGt = sccpParamFactory.createGlobalTitle(getLocalGt(),
        ...
        var callingParty = sccpParamFactory.createSccpAddress(RoutingIndicator.ROUTING_BASED_ON_GLOBAL_TITLE, callingGt,
    ...
    public void addToCarrier(MAPDialogMobility dialog) throws SystemException {
        try {
            ...
            var gsmScfGt = StringUtils.isBlank(getGsmSCFGt()) ? getLocalGt() : getGsmSCFGt();
            var gsmSCF = paramFactory.createISDNAddressString(ADDRESS_NATURE, ISDN_TELEPHONY_PLAN, gsmScfGt);
            ISDNAddressString msisdn = null;
            IMSI imsi = null;
            if (StringUtils.isNotBlank(getMsisdn())) {
                msisdn = paramFactory.createISDNAddressString(ADDRESS_NATURE, ISDN_TELEPHONY_PLAN,
                        getMsisdn());
            } else {
                imsi = paramFactory.createIMSI(getImsi());
            }
            var subscriberIdentity = (msisdn != null) ? paramFactory.createSubscriberIdentity(msisdn) :
                    paramFactory.createSubscriberIdentity(imsi);
            var requestedInfo = paramFactory.createRequestedInfo(true, true,
                    null, true, DomainType.csDomain, true,
                    false, false);
            dialog.addAnyTimeInterrogationRequest(subscriberIdentity, requestedInfo, gsmSCF, null);
    ...
    protected MAPApplicationContext getApplicationContext() throws SystemException {
        ...
        return MAPApplicationContext.getInstance(MAPApplicationContextName.anyTimeEnquiryContext, mapContextVersion);
    }
}
