...
@Payload(name = Constants.REPORT_SM_DELIVERY_STATUS_PAYLOAD_NAME)
public class ReportSmDeliveryStatusPayloadWrapper extends JSs7PayloadWrapper< MapSmsService, MAPDialogSms> {
    ...
    @Builder
    public ReportSmDeliveryStatusPayloadWrapper(String localGt, int localSsn, int remoteSsn, NodeConfig nodeConfig, 
                                                SccpAdapter sccpAdapter, MapAdapter mapAdapter, 
                                                MapDialogGenerator<MAPDialogSms> dialogGenerator, 
                                                String msisdn, String targetHlrGt, String smscGt, String mapVersion) {
    ...
    public MAPDialogSms generateCarrier() throws SystemException {
        ...
        var callingGt = sccpFactory.createGlobalTitle(getLocalGt(), TRANSLATION_TYPE, ISDN_TELEPHONY_INDICATOR,
        ...
        var calledGtStr = StringUtils.isBlank(getTargetHlrGt()) ? getMsisdn() :
                getTargetHlrGt();
        var calledGt = sccpFactory.createGlobalTitle(calledGtStr,
    ...
    public void addToCarrier(MAPDialogSms dialog) throws SystemException {
        try {
            ...
            var msisdn = getMapAdapter().getParamFactory().createISDNAddressString(AddressNature.international_number,
                            NumberingPlan.ISDN, getMsisdn());
            var callingSmscAddress = StringUtils.isBlank(getSmscGt()) ? getLocalGt() :
                    getSmscGt();
            var serviceCentreAddress = getMapAdapter().getParamFactory()
                    .createISDNAddressString(AddressNature.international_number, NumberingPlan.ISDN, callingSmscAddress);

            dialog.addReportSMDeliveryStatusRequest(msisdn, serviceCentreAddress, SMDeliveryOutcome.successfulTransfer,
