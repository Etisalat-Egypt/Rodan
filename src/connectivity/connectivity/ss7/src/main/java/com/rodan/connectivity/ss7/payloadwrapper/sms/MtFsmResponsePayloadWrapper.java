...
@Payload(name = Constants.MT_FSM_RESPONSE_PAYLOAD_NAME)
public class MtFsmResponsePayloadWrapper extends JSs7PayloadWrapper<MapSmsService, MAPDialogSms> {
    @Getter(AccessLevel.PRIVATE) private Long invokeId;

    @Builder
    public MtFsmResponsePayloadWrapper(String localGt, int localSsn, int remoteSsn, NodeConfig nodeConfig,
    ...
    public void addToCarrier(MAPDialogSms dialog) throws SystemException {
        try {
            ...
            dialog.addMtForwardShortMessageResponse(invokeId, null, null);

        } catch (MAPException e) {
            var msg = "Failed to add MT-FSM-Response to dialog";
            logger.error(msg, e);
            throw SystemException.builder().code(ErrorCode.MAP_INITIALIZATION).message(msg).build();
        }
    }
